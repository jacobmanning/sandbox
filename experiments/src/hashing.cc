...
unsigned long get_string_and_hash() {
  ...
  std::cout << "You entered: " << my_string << std::endl;
...
int main() {
  ...
  std::cout << "Hashed: " << my_hash << std::endl;

  // Write the user's hash to the file
  if (!write_hash_to_file(hash_filename, my_hash)) {
    std::cerr << "Could not write hash to file: " << hash_filename << std::endl;
    return 1;
  }
  ...
  if (!hash_from_file) {
    std::cerr << "Could not read hash from file: " << hash_filename << std::endl;
    return 1;
  }
