...
template <typename T>
std::pair<typename DirectedGraph<T>::iterator, bool> DirectedGraph<T>::insert(T&& node_value)
{
...
template <typename T>
std::pair<typename DirectedGraph<T>::iterator, bool> DirectedGraph<T>::insert(const T& node_value)
{
...
template <typename T>
typename DirectedGraph<T>::iterator DirectedGraph<T>::erase(const_iterator first,
                                                            const_iterator last)
{
...
bool DirectedGraph<T>::operator==(const DirectedGraph<T>& rhs) const
{
  for (auto&& node : nodes_)
  {
    ...
    const auto adjacent_values_lhs = get_adjacent_node_values(node.get_adjacent_node_indices());
...
template <typename T>
typename DirectedGraph<T>::reference DirectedGraph<T>::
operator[](typename DirectedGraph<T>::size_type index)
{
  return nodes_[index].get();
}

template <typename T>
typename DirectedGraph<T>::const_reference DirectedGraph<T>::
operator[](typename DirectedGraph<T>::size_type index) const
{
  return nodes_[index].get();
}

template <typename T>
typename DirectedGraph<T>::reference
DirectedGraph<T>::at(typename DirectedGraph<T>::size_type index)
{
  return nodes_.at(index).get();
}

template <typename T>
typename DirectedGraph<T>::const_reference
DirectedGraph<T>::at(typename DirectedGraph<T>::size_type index) const
{
  return nodes_.at(index).get();
}
...
DirectedGraph<T>::end(const T& node_value) const noexcept
{
  ...
  return adjacent_nodes_iterator<DirectedGraph>(std::end(iter->get_adjacent_node_indices()), this);
}
...
{
  return std::find_if(std::begin(nodes_), std::end(nodes_), [&node_value](const auto& node) {
...
template <typename T>
void DirectedGraph<T>::remove_all_links_to(typename nodes_container_type::const_iterator node)
{
  ...
  for (auto&& node : nodes_)
  {
    ...
    for (auto iter = std::begin(adjacent_indices); iter != std::end(adjacent_indices);)
    {
